...
In order to succesfully integrate a new system component in an existing system architecture, flows of information must be aligned with adjacent system components, so that data dependencies of the new component are satisfied, and expected functionalities can be provided in return. The technologies and methods that are going to be used are depending on these decisions, but must be looked at in isolation for comparison.
...
\begin{itemize}
	...
	\item Example 2: The microservice connects to an exising databases to acquire user information.
...
\subsection{JSON Web Token Proposal}
...
\subsection{API Gateway Proposal}
